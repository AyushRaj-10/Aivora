...
async function processAudio(audioBuffer, history = [], useVoiceClone = true, onPartial = () => {}) {
  ...
  // Dump the raw WebM buffer sent from the Frontend to the local drive so Coqui TTS can read it.
  const tempPath = path.resolve(process.cwd(), '../voice_clone/temp_speaker.webm');
  fs.writeFileSync(tempPath, audioBuffer);
  logger.info(`[Pipeline] Wrote target audio to ${tempPath}`);
  ...
  const { reply, enhancedDialogue, enhanced_vad, enhanced_segments } = await generateResponse(text, vad, emotion, history, segments);
  logger.debug(`[Pipeline] Step 2 (LLM) done in ${Date.now() - step2Start}ms`);
  
  onPartial({ 
    type: 'llm_response', 
    reply: enhancedDialogue, 
    enhanced_vad, 
    enhanced_segments 
  });
  ...
  // ─── STEP 3: TTS (Voice Clone — may take 60-120s on CPU) ────────
  const step3Start = Date.now();
  if (useVoiceClone) {
    onPartial({ type: 'tts_progress', message: 'Cloning your voice with XTTS... this may take up to 2 minutes on CPU.' });
  }
  await synthesizeSpeech(enhancedDialogue, emotion, tempPath, useVoiceClone, (base64Chunk) => {
